...
class MockTerminal():
    def __init__(self,filename,*args):
        self.arg = args
        self.command = ["../venv/bin/python", "tmp.py",*args]
    ...
    def compile(self,file,output="tmp.py"):
        command = ["../venv/bin/python","../samc","gen",file,"-o",output]
...
class ScriptTest(unittest.TestCase):
    ...
    def test_print_sam(self):
        ...
        with MockTerminal("./script/print.sam") as (_,proc):
            proc.wait()
    ...
    def test_database_sam(self):
        try:
            ...
            print("[数据库脚本测试]")
            print("   [测试'3月账单']", end='')
            with MockTerminal("./script/database.sam","ruiqurm") as (write,proc):
                write("3月账单\n")
                proc.wait()
                result = proc.stdout.read()
            ...
            print("pass\n   [测试'查询a的id']", end='')
            with MockTerminal("./script/database.sam","a") as (write,proc):
                write("id\n")
                proc.wait()
                result = proc.stdout.read()
